<?php
	...
	*/
	...
	function getCaller($what = NULL) {
		...
		$count = count($trace);
		if ($count >=3) {
			$previousCall = $trace[2]['function']; // 0 is this call, 1 is call in previous function, 2 is caller of that function
		} else {
			$previousCall = $trace[$count - 1]['function'];
		}
		if(isset($what)) {
	...
	function metaTags() {
		...
		$tags .= '<meta id="tokenData" data="'.$_SESSION['token_plexserver'].'"></meta>'.
				...
				'<meta id="apiTokenData" data="'.$_SESSION['apiToken'].'"></meta>'.
 ...
 // Push API.ai bot to other's account.  This can go after Google approval
	...
	function checkSetApiToken() {
		...
		foreach ($_SESSION['config'] as $section => $user) {
			if ($section != "general") {
				if ($_SESSION['username'] = $user['plexUserName']) {
					...
					if (!($user['apiToken'])) {
						...
						write_log("User API Token Retrieved.");
					}
