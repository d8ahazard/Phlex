...
function initialize() {
	...
	if (isset($_POST['username']) && isset($_POST['password'])) {
		...
		if (isset($_POST['new'])) {
			...
			if ($_SESSION['newToken']) write_log("new fucking token found","INFO");
...
function parseFetchCommand($command,$type=false) {
	...
	if (! $type) {
		write_log("No type specified, let's ask the internet.");
			...
			if ($container) {
				...
				if (isset($container->Video)) {
					foreach ($container->Video as $video) {
						...
						write_log("ListItem: ".json_encode($item));
...
function metaTags() {
	write_log("Function fired: ".$_SESSION['newToken']);
	$tags = '';
	$filename = "commands.php";
	$handle = fopen($filename, "r");
	//Read first line, but do nothing with it
		...
		if ($scheduled) {
			...
			if (is_array($shows)) {
				foreach($shows as $show) {
					write_log("Sick show: ".json_encode($show));
	...
	if ($enableSonarr) {
		...
		if ($scheduled) {
			foreach ($scheduled as $show) {
				write_log("Sonarr show: ".json_encode($show));
				$extra = fetchTVDBInfo($show['tvdbId']);
