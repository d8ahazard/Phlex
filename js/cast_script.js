...
function setBackground() {
	...
	if (showFade == fade1) {
		showFade = fade2;
		hideFade = fade1;
		console.log("Fade 1 is visible.");
	} else {
		showFade = fade1;
		hideFade = fade2;
		console.log("Fade 2 is visible.");
	}
	console.log("Image should be set to " + imgUrl);
	showFade.find(".background").css('background-image','url('+imgUrl+')');
	showFade.find(".timeDiv").text(formatAMPM());
	showFade.find("#metadata-line-1").text(desc1);
	showFade.find("#metadata-line-2").text(desc2);
	showFade.find("#metadata-line-3").text(desc3);
	setWeather();
	showFade.fadeIn(1200);
	hideFade.fadeOut(1100);
	...
	$.get("http://freegeoip.net/json/", function(data) {
		...
		$.simpleWeather({
			...
			success: function(weather) {
				...
				console.log("Tempstring: " + html);
				showFade.find(".tempDiv").text(html);
				var condition = weather.code;
				console.log("Condition code: " + condition);
				var weatherClass;
				switch (condition) {
					case "0":
					case "1":
				...
				console.log("Weather Class is " + weatherClass);
